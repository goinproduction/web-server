...
router.post('/register', jsonParser, (req, res) => {
    ...
    User.findOne({ email }, (err, user) => {
        if (err) res.status(500).json("Error has occurred. Please refresh page")
            ...
            User.findOne({ username }, (err, user) => {
                if (err) res.status(500).json("Error has occurred. Please refresh page")
                    ...
                    newUser.save()
                        ...
                        .catch(err => res.status(500).json("Error has occurred. Please refresh page"));
                }
            })
        }
    })
...
router.post('/login', (req, res) => {
    ...
    //find the user
    User.findOne({ email }, (err, user) => {
        if (err) res.status(500).json("Error has occurred.");
        else if (!user) res.status(400).json("User not found");
        else {
            //check the password is correct
            user.comparePassword(password, (err, isMatch) => {
                if (err) res.status(500).json("Error is occurred.")
                if (isMatch) {
                    const token = generateToken();
                    user.token = token;
                    user.save();
